...
ssh %SSH_USER%@%SSH_HOST% "set -e; APP_DIR='%APP_DIR%'; BRANCH='%BRANCH%'; REPO_URL='%REPO_URL%'; mkdir -p \"$APP_DIR\"; if [ ! -f \"$APP_DIR/script_ai/deploy_server.sh\" ]; then if [ -n \"$(find \"$APP_DIR\" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null)\" ]; then BACKUP=\"$APP_DIR.bootstrap_backup.$$\"; mkdir -p \"$BACKUP\"; shopt -s dotglob nullglob; mv \"$APP_DIR\"/* \"$BACKUP\"/; shopt -u dotglob nullglob; fi; if echo \"$REPO_URL\" | grep -q '^git@github.com:'; then REPO_URL=\"https://github.com/${REPO_URL#git@github.com:}\"; fi; git clone --branch \"$BRANCH\" --single-branch \"$REPO_URL\" \"$APP_DIR\"; fi; cd \"$APP_DIR\"; APP_DIR=\"$APP_DIR\" REPO_URL=\"$REPO_URL\" bash script_ai/deploy_server.sh \"$BRANCH\""
if errorlevel 1 (
